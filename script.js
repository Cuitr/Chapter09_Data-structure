...
const camelCase = function(text){
    const textPart = text.toLowerCase().split(`_`);
    const textCamel = [];
    for(let i = 0; i < textPart.length; i++){
      if(i == 0){
        textCamel.push(textPart[i]);
      }else{
        textCamel.push(textPart[i].replace(textPart[i][0], textPart[i][0].toUpperCase()));
      }
    }
    return textCamel.join(``);
}
console.log(camelCase(`underscore_case`));
console.log(camelCase(`first_name`));
console.log(camelCase(`Some_Variable`));
console.log(camelCase(`cucalculate_AGE`));
